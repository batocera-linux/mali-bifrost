...
void kbase_csf_term_descheduled_queue_group(struct kbase_queue_group *group)
{
	...
	/* Currently each group supports the same number of CSs */
	u32 max_streams =
 ...
 * output page and pass them to the tiler heap function to allocate a new chunk.
 * It will also update the CS's kernel input page with the addressof a new chunk
 * that was allocated.
 *
 ...
 *
 * See process_cs_interrupts() for details of per-CS interrupt handling.
 */
